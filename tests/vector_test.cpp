...
namespace
{
    ...
    /**
     * @brief Verify that FGB files generated by buildVector have spatial index enabled.
     *
     * Spatial index is essential for efficient HTTP range request queries.
     * FlatGeobuf files with SPATIAL_INDEX=YES support OLCFastSpatialFilter capability.
     */
    TEST(testVector, spatialIndexVerification)
    {
        ...
        OGRLayerH hLayer = GDALDatasetGetLayer(hDS, 0);
        ASSERT_NE(hLayer, nullptr) << "Should have at least one layer";

        // FlatGeobuf with SPATIAL_INDEX=YES supports OLCFastSpatialFilter capability
        bool hasFastSpatialFilter = OGR_L_TestCapability(hLayer, OLCFastSpatialFilter);
        EXPECT_TRUE(hasFastSpatialFilter)
            << "FGB file should have spatial index (SPATIAL_INDEX=YES)";
