...
TEST(getIndexPathList, includeDirs) {
    auto pathList = ddb::getIndexPathList(
        "data", {(fs::path("data") / "folderA" / "test.txt").string()}, true);
    EXPECT_EQ(pathList.size(), 2);
    EXPECT_TRUE(std::find(pathList.begin(), pathList.end(),
                          fs::path("data") / "folderA" / "test.txt") !=
                pathList.end());
    EXPECT_TRUE(std::find(pathList.begin(), pathList.end(),
                          fs::path("data") / "folderA") != pathList.end());

    pathList = ddb::getIndexPathList(
        ".",
        {(fs::path("data") / "folderA" / "test.txt").string(),
         (fs::path("data") / "folderA" / "folderB" / "test.txt").string()},
        true);
    ...
    EXPECT_TRUE(std::find(pathList.begin(), pathList.end(),
                ...
                pathList.end());
    EXPECT_TRUE(std::find(pathList.begin(), pathList.end(),
                          fs::path("data") / "folderA" / "folderB" /
                              "test.txt") != pathList.end());
    EXPECT_TRUE(std::find(pathList.begin(), pathList.end(),
                          fs::path("data") / "folderA") != pathList.end());
    EXPECT_TRUE(std::find(pathList.begin(), pathList.end(), fs::path("data")) !=
                pathList.end());
    EXPECT_TRUE(std::find(pathList.begin(), pathList.end(),
                          fs::path("data") / "folderA" / "folderB") !=
                pathList.end());

    EXPECT_THROW(pathList = ddb::getIndexPathList(
                     "otherRoot",
                     {
                         (fs::path("data") / "folderA" / "test.txt").string(),
                     },
...
TEST(getIndexPathList, dontIncludeDirs) {
    auto pathList = ddb::getIndexPathList(
        "data", {(fs::path("data") / "folderA" / "test.txt").string()}, false);
    EXPECT_EQ(pathList.size(), 1);
    EXPECT_STREQ(pathList[0].string().c_str(),
                 (fs::path("data") / "folderA" / "test.txt").string().c_str());
}
...
TEST(deleteFromIndex, fileExactInFolder) {
    ...
    toRemove.emplace_back(
        (testFolder / "pics" / "IMG_20160826_181309.jpg").string());
...
TEST(deleteFromIndex, fileExactDirtyDotDot) {
    ...
    // 1
    toRemove.emplace_back(
        (testFolder / "pics" / ".." / "1JI_0065.JPG").string());
...
TEST(listIndex, allFileWildcard) {
    ...
    EXPECT_EQ(
        out.str(),
        "1JI_0064.JPG\n1JI_0065.JPG\npics\npics.JPG\npics/"
        "IMG_20160826_181302.jpg\npics/IMG_20160826_181305.jpg\npics/"
        "IMG_20160826_181309.jpg\npics/IMG_20160826_181314.jpg\npics/"
        "IMG_20160826_181317.jpg\npics/pics2\npics2\npics2/"
        "IMG_20160826_181305.jpg\npics2/IMG_20160826_181309.jpg\npics2/pics\n");
...
TEST(listIndex, fileExactInSubFolderDetails) {
    ...
    const auto sqlite = ta.downloadTestAsset(
        "https://github.com/DroneDB/test_data/raw/master/registry/"
        "DdbFactoryTest/dbase.sqlite",
        "dbase.sqlite");
...
TEST(listIndex, fileExactInSubfolder) {
    ...
    toList.emplace_back(
        (testFolder / "pics" / "IMG_20160826_181314.jpg").string());
...
TEST(listIndex, fileExactInSubfolderWithPathToResolve) {
    ...
    toList.emplace_back(
        (testFolder / "pics2" / ".." / "pics" / "IMG_20160826_181314.jpg")
            .string());
...
TEST(loadPointGeom, jsonOk) {
    ...
    std::string text =
        R"({"type":"Point","coordinates":[-91.994560,46.842607,198.31]})";
...
TEST(loadPointGeom, wrongType) {
    BasicPointGeometry point_geom;
    std::string text =
        R"({"type":"Polygon","coordinates":[-91.994560,46.842607,198.31]})";

    EXPECT_THROW(loadPointGeom(&point_geom, text), DBException);
}

TEST(loadPointGeom, wrongNumberOfCoordinates) {
    BasicPointGeometry point_geom;
    std::string text =
        R"({"type":"Point","coordinates":[-91.994560,-91.994560,46.842607,198.31]})";
...
TEST(loadPolygonGeom, jsonOk) {
    ...
    std::string text =
        R"({"type":"Polygon","coordinates":[[[-91.99469773385999,46.84296499722999,158.5100007629],[-91.99507616866998,46.84271189348,158.5100007629],[-91.9944204067,46.84225026546,158.5100007629],[-91.99404197212,46.84250336707,158.5100007629],[-91.99469773385999,46.84296499722999,158.5100007629]]]})";
    ...
    EXPECT_EQ(
        point_geom.toWkt(),
        "POLYGONZ ((-91.99469773386 46.84296499723 158.5100007629, "
        "-91.99507616867 46.84271189348 158.5100007629, -91.9944204067 "
        "46.84225026546 158.5100007629, -91.99404197212 46.84250336707 "
        "158.5100007629, -91.99469773386 46.84296499723 158.5100007629))");
}
...
TEST(loadPolygonGeom, wrongType) {
    ...
    EXPECT_THROW(loadPolygonGeom(&point_geom, text), DBException);
}

TEST(loadPolygonGeom, wrongNumberOfCoordinates) {
    BasicPolygonGeometry point_geom;
    std::string text =
        R"({"type":"Polygon","coordinates":[[[-91.99469773385999,46.84296499722999,46.84296499722999,158.5100007629],[-91.99507616866998,46.84271189348,158.5100007629],[-91.9944204067,46.84225026546,158.5100007629],[-91.99404197212,46.84250336707,158.5100007629],[-91.99469773385999,46.84296499722999,158.5100007629]]]})";
