...
TEST(pathHasChildren, Normal) {
    EXPECT_TRUE(
        io::Path("/my/path").hasChildren({"/my/path/1", "/my/path/a/b/.."}));

#ifdef _WIN32
    EXPECT_TRUE(
        io::Path("C:\\my\\path")
            .hasChildren({"C:\\my\\path\\1", "C:\\my\\path\\a\\b\\.."}));
#endif

    EXPECT_TRUE(
        io::Path("path").hasChildren({"path/1/2", "path/3", "path/././6"}));
    EXPECT_TRUE(io::Path("path/./").hasChildren(
        {"path/1/2", "path/3/", "path/./6/7/../"}));

#ifdef _WIN32
    EXPECT_TRUE(io::Path("path\\.").hasChildren(
        {"path\\1\\2", "path\\3", "path\\4\\"}));
#endif
...
TEST(pathDepth, Normal) {
    ...
    EXPECT_EQ(
        io::Path((fs::current_path().root_path() / "a" / "file.txt")).depth(),
        1);
    EXPECT_EQ(
        io::Path((fs::current_path().root_path() / "a" / "b" / "file.txt"))
            .depth(),
        2);
    EXPECT_EQ(io::Path(".").depth(), 0);
...
TEST(pathRelativeTo, Normal) {
...
#endif
    EXPECT_EQ(
        io::Path("/home/test/aaa/bbb/ccc/../..").relativeTo("/home").generic(),
    ...
    EXPECT_EQ(io::Path("D:\\").relativeTo("/").generic(),
              io::Path("").generic());
#else
    EXPECT_EQ(io::Path("/").relativeTo("/").generic(), io::Path("").generic());
#endif
...
TEST(pathCheckExtension, Normal) {
    ...
    EXPECT_FALSE(
        io::Path("/home/test.jpeg.tif").checkExtension({"JpG", "jpEG"}));
...
TEST(getModifiedTime, Normal) {
    ...
    // Works on files
    EXPECT_TRUE(io::Path(io::getDataPath("timezone21.bin")).getModifiedTime() >
                0);
}
...
TEST(commonDirPath, Normal) {
    EXPECT_EQ(io::commonDirPath({"/test/123", "/test/abc"}).generic_string(),
              "/test");
    ...
    EXPECT_EQ(
        io::commonDirPath({"abc/abc/test.txt", "abc", "def"}).generic_string(),
        "");

    EXPECT_EQ(io::commonDirPath({"abc/abc/test.txt", "abc/abc/test2.txt"})
                  .generic_string(),
              "abc/abc");
    EXPECT_EQ(io::commonDirPath({"/abc"}).generic_string(), "/abc");

#ifdef _WIN32
    EXPECT_EQ(io::commonDirPath({"d:\\test\\123", "d:\\test\\abc"}).string(),
              "d:\\test");
    EXPECT_EQ(io::commonDirPath({"c:\\test\\123", "c:\\test2\\abc"}).string(),
              "c:\\");
