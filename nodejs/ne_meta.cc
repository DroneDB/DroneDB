...
public:
    MetaAddWorker(Nan::Callback* callback,
    ...
    void Execute() {
        if (DDBMetaAdd(ddbPath.c_str(), path.c_str(), key.c_str(), data.c_str(), &output) !=
            DDBERR_NONE) {
...
public:
    MetaSetWorker(Nan::Callback* callback,
                  const std::string& ddbPath,
                  const std::string& path,
                  const std::string& key,
                  const std::string& data)
    ...
    void Execute() {
        if (DDBMetaSet(ddbPath.c_str(), path.c_str(), key.c_str(), data.c_str(), &output) !=
            DDBERR_NONE) {
            SetErrorMessage(DDBGetLastError());
        }
    }
...
public:
    MetaRemoveWorker(Nan::Callback* callback, const std::string& ddbPath, const std::string& id)
        : AsyncWorker(callback, "nan:MetaRemoveWorker"), ddbPath(ddbPath), id(id) {}
    ~MetaRemoveWorker() {}
...
public:
    MetaGetWorker(Nan::Callback* callback,
                  const std::string& ddbPath,
                  const std::string& path,
                  const std::string& key)
        : AsyncWorker(callback, "nan:MetaGetWorker"), ddbPath(ddbPath), path(path), key(key) {}
    ~MetaGetWorker() {}
...
public:
    MetaUnsetWorker(Nan::Callback* callback,
                    const std::string& ddbPath,
                    const std::string& path,
                    const std::string& key)
        : AsyncWorker(callback, "nan:MetaUnsetWorker"), ddbPath(ddbPath), path(path), key(key) {}
    ~MetaUnsetWorker() {}
...
public:
    MetaListWorker(Nan::Callback* callback, const std::string& ddbPath, const std::string& path)
        : AsyncWorker(callback, "nan:MetaListWorker"), ddbPath(ddbPath), path(path) {}
    ~MetaListWorker() {}
...
public:
    MetaDumpWorker(Nan::Callback* callback, const std::string& ddbPath, const std::string& ids)
        : AsyncWorker(callback, "nan:MetaDumpWorker"), ddbPath(ddbPath), ids(ids) {}
    ~MetaDumpWorker() {}
