...
inline std::vector<std::string> v8ArrayToStrVec(
...
#define BIND_OBJECT_VAR(__obj, __type, __name, __defaultValue)                 \
    v8::Local<v8::String> __name##key =                                        \
        Nan::New<v8::String>(#__name).ToLocalChecked();                        \
    __type __name = __defaultValue;                                            \
    if (Nan::HasRealNamedProperty(__obj, __name##key).FromJust()) {            \
        __name = Nan::To<__type>(Nan::GetRealNamedProperty(__obj, __name##key) \
                                     .ToLocalChecked())                        \
                     .FromJust();                                              \
    }
...
#define BIND_OBJECT_STRING(__obj, __name, __defaultValue)                 \
    ...
    if (Nan::HasRealNamedProperty(__obj, __name##key).FromJust()) {       \
        Nan::Utf8String str(Nan::GetRealNamedProperty(__obj, __name##key) \
                                .ToLocalChecked()                         \
                                .As<v8::String>());                       \
...
#define BIND_FUNCTION_PARAM(__name, __num)                         \
    ...
    Nan::Callback *__name = new Nan::Callback(                     \
        Nan::To<v8::Function>(info[__num]).ToLocalChecked());
...
#define BIND_UNSIGNED_INT_PARAM(__name, __num)                   \
    ...
    uint32_t __name =                                            \
        Nan::To<uint32_t>(info[__num].As<v8::Uint32>()).FromJust();
