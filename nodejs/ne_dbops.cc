...
class AddWorker : public Nan::AsyncWorker {
   public:
    AddWorker(Nan::Callback *callback, const std::string &ddbPath,
    ...
    void Execute() {
        ...
        if (DDBAdd(ddbPath.c_str(), cPaths.data(),
                   static_cast<int>(cPaths.size()), &output,
                   recursive) != DDBERR_NONE) {
            SetErrorMessage(DDBGetLastError());
        }
    }
    ...
    void HandleOKCallback() {
        ...
        v8::Local<v8::Value> argv[] = {
            Nan::Null(),
            json.Parse(Nan::New<v8::String>(output).ToLocalChecked())
                .ToLocalChecked()};
        delete output;  // TODO: is this a leak if the call fails? How do we
                        // de-allocate on failure?
        callback->Call(2, argv, async_resource);
    }
...
class RemoveWorker : public Nan::AsyncWorker {
   public:
    RemoveWorker(Nan::Callback *callback, const std::string &ddbPath,
...
class ListWorker : public Nan::AsyncWorker {
   public:
    ListWorker(Nan::Callback *callback, const std::string &ddbPath,
               const std::vector<std::string> &paths, bool recursive,
    ...
    void Execute() {
        ...
        if (DDBList(ddbPath.c_str(), cPaths.data(),
                    static_cast<int>(cPaths.size()), &output, "json", recursive,
...
NAN_METHOD(list) {
    ...
    Nan::AsyncQueueWorker(
        new ListWorker(callback, ddbPath, in, recursive, maxRecursionDepth));
}