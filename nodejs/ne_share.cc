...
public:
    ShareWorker(Nan::Callback* callback,
    ...
    void Execute(const Nan::AsyncProgressWorker::ExecutionProgress& progress) {
        ddb::ShareService ss;
        try {
            ddb::ShareCallback showProgress = [&](const std::vector<ddb::ShareFileProgress*>& files,
                                                  size_t txBytes,
                                                  size_t totalBytes) {
                // Prepare JSON object
                json fa = json::array();
                ...
                json j = {{"files", fa}, {"txBytes", txBytes}, {"totalBytes", totalBytes}};
    ...
    void HandleOKCallback() {
        ...
        v8::Local<v8::Value> argv[] = {Nan::Null(), Nan::New(url).ToLocalChecked()};
