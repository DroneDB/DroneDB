...
class ShareWorker : public Nan::AsyncProgressWorker {
   public:
    ShareWorker(Nan::Callback *callback, Nan::Callback *progress,
                const std::vector<std::string> &input, const std::string &tag,
        ...
        : Nan::AsyncProgressWorker(callback, "nan:ShareWorker"),
          ...
          recursive(recursive),
          cancel(false) {}
    ~ShareWorker() { delete progress; }

    void Execute(const Nan::AsyncProgressWorker::ExecutionProgress &progress) {
        ddb::ShareService ss;
        try {
            ddb::ShareCallback showProgress =
                [&](const std::vector<ddb::ShareFileProgress *> &files,
    ...
    void HandleProgressCallback(const char *data, size_t count) {
        ...
        v8::Local<v8::Value> argv[] = {
            json.Parse(Nan::New<v8::String>(str).ToLocalChecked())
                .ToLocalChecked()};
...
NAN_METHOD(share) {
    ...
    Nan::AsyncQueueWorker(
        new ShareWorker(callback, progress, paths, tag, password, recursive));
}
