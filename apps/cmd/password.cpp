	...
	void Password::run(cxxopts::ParseResult &opts)
	{
		...
		try
		{
			...
			if (command == "a" || command == "append")
			{
				if (argument.empty())
				{
					std::cout << "Missing parameter in append command" << std::endl
							  << std::endl;
					printHelp();
				}
				...
				std::cout << std::endl
						  << "Password added to database" << std::endl;
			}
			else if (command == "v" || command == "verify")
			{
				...
				std::cout << std::endl
						  << (res ? "Password verification succeeded" : "Password verification failed") << std::endl;
				if (!res)
					exit(EXIT_FAILURE);
			}
