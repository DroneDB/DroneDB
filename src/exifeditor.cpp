...
ExifEditor::ExifEditor(const std::string &file) { files.push_back(file); }
    ...
    for (auto &file : files) {
        try {
            ...
            if (fs::is_directory(file))
                throw FSException("cannot set EXIFs to a directory");
...
void ExifEditor::SetGPSAltitude(double altitude) {
    eachFile([=](const fs::path &f, Exiv2::ExifData &exifData) {
        ...
        LOGD << "Setting altitude to "
             << exifData["Exif.GPSInfo.GPSAltitude"].toString() << " ("
             << exifData["Exif.GPSInfo.GPSAltitudeRef"].toString() << ") for "
             << f.string();
...
void ExifEditor::SetGPS(double latitude, double longitude, double altitude) {
    eachFile([=](const fs::path &f, Exiv2::ExifData &exifData) {
        ...
        LOGD << "Setting lat: "
             << exifData["Exif.GPSInfo.GPSLatitude"].toString() << " "
             << exifData["Exif.GPSInfo.GPSLatitudeRef"].toString() << " "
             << "lon: " << exifData["Exif.GPSInfo.GPSLongitude"].toString()
             << " " << exifData["Exif.GPSInfo.GPSLongitudeRef"].toString()
             << " "
             << "alt: " << exifData["Exif.GPSInfo.GPSAltitude"].toString()
             << " (" << exifData["Exif.GPSInfo.GPSAltitudeRef"].toString()
             << ") "
             << "for " << f.string();
    });
}
...
const std::string ExifEditor::doubleToDMS(double d) {
    ...
    return std::to_string(deg) + "/1 " + std::to_string(min) + "/1 " +
