...
float DSMService::getAltitude(double latitude, double longitude) {
    ...
    for (auto &it : cache) {
        if (it.second.bbox.contains(point)) {
            ...
            if (!it.second.hasNodata ||
                !utils::sameFloat(elevation, it.second.nodata)) {
                return elevation;
            } else {
    ...
    if (addGeoTIFFToCache(loadFromNetwork(latitude, longitude), latitude,
                          longitude)) {
        return getAltitude(latitude, longitude);
    }
...
std::string DSMService::loadFromNetwork(double latitude, double longitude) {
    ...
    std::string url = std::regex_replace(format, std::regex("\\{west\\}"),
                                         std::to_string(min.longitude));
    url = std::regex_replace(url, std::regex("\\{east\\}"),
                             std::to_string(max.longitude));
    url = std::regex_replace(url, std::regex("\\{north\\}"),
                             std::to_string(max.latitude));
    url = std::regex_replace(url, std::regex("\\{south\\}"),
                             std::to_string(min.latitude));

    // Try to download
    std::string filename =
        std::to_string(static_cast<int>(p.x)) + "_" +
...
bool DSMService::addGeoTIFFToCache(const fs::path &filePath, double latitude,
                                   double longitude) {
    GDALDataset *dataset = static_cast<GDALDataset *>(
        GDALOpen(filePath.string().c_str(), GA_ReadOnly));
    if (dataset == nullptr)
    ...
    if (wkt.empty())
        throw GDALException("Cannot get projection ref for " +
                            filePath.string());
    char *wktp = const_cast<char *>(wkt.c_str());
    // OGRSpatialReference *srs = new OGRSpatialReference();
    // if (srs->importFromWkt(&wktp) != OGRERR_NONE) throw GDALException("Cannot
    // read spatial reference system for " + filePath.string() + ". Is PROJ
    // installed?");
    ...
    // if (!srs->IsSame(compare)) throw GDALException("Cannot read DSM values
    // from raster: " + filePath.string() + " (EPSG != 4326)");
    if (dataset->GetRasterCount() != 1)
        throw GDALException(
            "More than 1 raster band found in elevation raster: " +
            filePath.string());

    e.nodata = static_cast<float>(
        dataset->GetRasterBand(1)->GetNoDataValue(&e.hasNodata));
    ...
    if (contained) {
        ...
        LOGD << position << " inside raster boundary, loading data from "
             << filePath.string();
        e.loadData(dataset);
    }
...
float DSMCacheEntry::getElevation(double latitude, double longitude) {
    if (data.empty())
        throw AppException(
            "Cannot get elevation, need to call loadData() first.");
    if (width == 0 || height == 0)
        throw AppException(
            "Cannot get elevation, need to populate width/height first.");
    ...
    unsigned int pixelX =
        static_cast<unsigned int>((longitude - originX) / pixelSizeX);
    unsigned int pixelY =
        static_cast<unsigned int>((latitude - originY) / pixelSizeY);

    if (pixelX > width || pixelY > height) {
        throw AppException("Pixel coordinates (" + std::to_string(pixelX) +
                           "," + std::to_string(pixelY) + ") " +
                           "are outside of raster boundaries (" +
