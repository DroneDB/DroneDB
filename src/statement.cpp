...
Statement::Statement(sqlite3 *db, const std::string &query)
    ...
    if (sqlite3_prepare_v2(db, query.c_str(), static_cast<int>(query.length()),
...
Statement &Statement::bind(int paramNum, const std::string &value) {
    ...
    bindCheck(sqlite3_bind_text(stmt, paramNum, value.c_str(),
...
Statement &Statement::step() {
    ...
    switch (code) {
        ...
        case SQLITE_MISUSE:
        case SQLITE_BUSY:
            throw DBException("Cannot execute step for " + query +
...
std::string Statement::getText(int columnId) {
    assert(stmt != nullptr);
    const auto res =
        reinterpret_cast<const char *>(sqlite3_column_text(stmt, columnId));
