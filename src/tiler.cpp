...
int Tiler::dataBandsCount(const GDALDatasetH &dataset) {
    ...
    int bandsCount = GDALGetRasterCount(dataset);
...
std::string Tiler::tile(int tz, int tx, int ty) {
    ...
    GDALDatasetH dsTile = GDALCreate(memDrv, "", tileSize, tileSize, nBands + 1,
                                     GDT_Byte, nullptr);
    ...
    int querySize = tileSize;  // TODO: you will need to change this for
...
fs::path TilerHelper::getCacheFolderName(const fs::path &geotiffPath,
...
fs::path TilerHelper::getFromUserCache(const fs::path &geotiffPath, int tz,
                                       int tx, int ty, int tileSize, bool tms,
                                       bool forceRecreate) {
    if (std::rand() % 1000 == 0) cleanupUserCache();
    if (!fs::exists(geotiffPath))
        throw FSException(geotiffPath.string() + " does not exist");

    time_t modifiedTime = io::Path(geotiffPath).getModifiedTime();
    fs::path tileCacheFolder =
        UserProfile::get()->getTilesDir() /
        getCacheFolderName(geotiffPath, modifiedTime, tileSize);
