...
[[noreturn]] void onError(int errZD, int errNative) {
    throw TimezoneException(
        "Timezone error: " + std::string(ZDGetErrorString(errZD)) + " (" +
...
void Timezone::init() {
    ...
    if (!db)
        throw TimezoneException(
            "Cannot open timezone database ./timezone21.bin");
...
cctz::time_zone Timezone::lookupTimezone(double latitude, double longitude) {
    ...
    ZoneDetectResult *results =
        ZDLookup(db, static_cast<float>(latitude),
                 static_cast<float>(longitude), &safezone);
    if (!results) {
    ...
    while (results[index].lookupResult != ZD_LOOKUP_END) {
        if (results[index].data) {
            std::string timezoneId = std::string(results[index].data[0]) +
                                     std::string(results[index].data[1]);

            if (!cctz::load_time_zone(timezoneId, &tz)) {
                LOGD << "Cannot load timezone, defaulting to UTC: "
                     << timezoneId;
            } else {
                found = true;
                break;
...
double Timezone::getUTCEpoch(int year, int month, int day, int hour, int minute,
                             int second, double msecs,
                             const cctz::time_zone &tz) {
    auto time =
        tz.lookup(cctz::civil_second(year, month, day, hour, minute, second));
