...
void info(const std::vector<std::string> &input, std::ostream &output,
          const std::string &format, bool recursive, int maxRecursionDepth,
          const std::string &geometry, bool withHash, bool stopOnError) {
    ...
    for (auto &fp : filePaths) {
        ...
        if (parseEntry(fp, "/", e, withHash, stopOnError)) {
            ...
            if (format == "json") {
                ...
                json j;
                if (e.toGeoJSON(j,
                                ddb::getBasicGeometryTypeFromName(geometry))) {
                    if (!first) output << ",";
                    output << j.dump();
                    j.clear();
                } else {
                    LOGD << "No geometries in " << fp.string()
                         << ", skipping from GeoJSON export";
                }
            } else {
                output << e.toString() << "\n";
            }
