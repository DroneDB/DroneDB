...
struct Projected2D_t {
    ...
    DDB_DLL void rotate(const Projected2D_t& center, double degrees) {
        ...
        x = cos(radians) * (px - center.x) - sin(radians) * (py - center.y) +
            center.x;
        y = sin(radians) * (px - center.x) + cos(radians) * (py - center.y) +
            center.y;
    }
...
struct BoundingBox {
    ...
    DDB_DLL bool contains(const T& p) { return contains(p.x, p.y); }
    template <typename N>
...
struct Geographic2D {
    ...
    DDB_DLL Geographic2D(double longitude, double latitude)
...
DDB_DLL Projected2D toUTM(double latitude, double longitude,
                          const UTMZone& zone);
