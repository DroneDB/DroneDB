...
void Share::setOptions(cxxopts::Options &opts) {
    opts.positional_help("[args]")
        ...
        .add_options()
            ...
            ("i,input", "Files and directories to share",
             cxxopts::value<std::vector<std::string>>())(
                "r,recursive", "Recursively share subdirectories",
                cxxopts::value<bool>())(
                "t,tag",
                "Tag to use (organization/dataset or "
                "server[:port]/organization/dataset)",
                cxxopts::value<std::string>()->default_value(
                    DEFAULT_REGISTRY "/<username>/<uuid>"))(
                "p,password", "Optional password to protect dataset",
                cxxopts::value<std::string>()->default_value(""))(
                "s,server",
                "Registry server to share dataset with (alias of: -t "
                "<server>//)",
            ...
            if (files.size() > 0) {
                ...
                float progress = f->txBytes > 0
                                     ? static_cast<float>(f->txBytes) /
                                           static_cast<float>(f->totalBytes) *
                                           100.0f
                                     : 0.0f;
