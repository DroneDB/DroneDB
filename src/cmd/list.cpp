...
std::string List::description() { return "List indexed files and directories"; }

void List::run(cxxopts::ParseResult& opts) {
    try {
        const auto ddbPath = opts["working-dir"].as<std::string>();
        const auto paths = opts.count("input") > 0
                               ? opts["input"].as<std::vector<std::string>>()
                               : std::vector<std::string>();
        const auto format = opts["format"].as<std::string>();
        const auto recursive = opts["recursive"].count() > 0;
        const auto depthOpt = opts["depth"];

        // Take into consideration maxRecursionDepth only if the recursive flag
        // is set and the option exists
        const auto maxRecursionDepth =
            recursive ? (depthOpt.count() > 0 ? depthOpt.as<int>() : 0) : 0;
        ...
        if (opts.count("output")) {
            ...
            listIndex(db.get(), paths, file, format, recursive,
                      maxRecursionDepth);

            file.close();
        } else {
            listIndex(db.get(), paths, std::cout, format, recursive,
                      maxRecursionDepth);
        }
