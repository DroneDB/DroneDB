...
void Meta::setOptions(cxxopts::Options &opts) {
    ...
    ("j,json", "JSON string to set", cxxopts::value<std::string>()->default_value(""))
    ("w,working-dir", "Working directory", cxxopts::value<std::string>()->default_value("."));

    // clang-format on
    opts.parse_positional({"command", "key", "json"});
}
...
void Meta::run(cxxopts::ParseResult &opts) {
    ...
    const auto json = opts["json"].as<std::string>();
    ...
    if (command == "add" || command == "set"){
        if (json.empty()) throw ddb::InvalidArgsException("No JSON specified");
    }

    if (command == "add"){
        const auto key = opts["key"].as<std::string>();
        std::cout << metaManager.add(key, json, path) << std::endl;
    }
}
