...
time_t Path::getModifiedTime() {
    ...
    FILETIME ftModified;
    hFile =
        CreateFile(p.string().c_str(), 0, FILE_SHARE_READ, NULL, OPEN_EXISTING,
...
bool Path::isAbsolute() const { return p.is_absolute(); }

bool Path::isRelative() const { return p.is_relative(); }
...
Path Path::relativeTo(const fs::path &parent) {
    ...
    fs::path relPath = relative(weakly_canonical(absolute(p)),
                                weakly_canonical(absolute(parent)));
...
std::string Path::string() const { return p.string(); }
...
fs::path getExeFolderPath() {
    ...
    return fs::path(std::string(result,
    ...
    while (maxComponents > 0) {
        for (size_t i = matchingComponents; i < maxComponents; i++) {
            for (auto it = paths.begin() + 1; it != paths.end(); it++) {
                if (std::next(it->begin(), i)->string() !=
                    std::next(smallest.begin(), i)->string()) {
                    smallest = smallest.parent_path();
                    maxComponents--;
                    goto breakout;  // Break out of two loops.
                                    // https://xkcd.com/292/
                }
            }
