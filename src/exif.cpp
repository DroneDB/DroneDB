...
Exiv2::ExifData::const_iterator ExifParser::findExifKey(
...
Exiv2::XmpData::const_iterator ExifParser::findXmpKey(
    const std::initializer_list<std::string> &keys) {
    for (auto &k : keys) {
        try {
...
ImageSize ExifParser::extractImageSize() {
    auto imgWidth =
        findExifKey({"Exif.Photo.PixelXDimension", "Exif.Image.ImageWidth"});
    auto imgHeight =
        findExifKey({"Exif.Photo.PixelYDimension", "Exif.Image.ImageLength"});

    // TODO: fallback on actual image size

    if (imgWidth != exifData.end() && imgHeight != exifData.end()) {
        return ImageSize(static_cast<int>(imgWidth->toLong()),
                         static_cast<int>(imgHeight->toLong()));
    } else {
...
bool ExifParser::extractSensorSize(SensorSize &r) {
    ...
    if (fUnit != exifData.end() && fXRes != exifData.end() &&
        fYRes != exifData.end()) {
...
bool ExifParser::extractRelAltitude(double &relAltitude) {
    ...
    if (extractGeo(geo) && geo.altitude > 0) {
        relAltitude =
            geo.altitude - static_cast<double>(DSMService::get()->getAltitude(
...
inline double ExifParser::geoToDecimal(
    const Exiv2::ExifData::const_iterator &geoTag,
    const Exiv2::ExifData::const_iterator &geoRefTag) {
    ...
    //    LOGD << geoTag->toRational(0).first << "/" <<
    //    geoTag->toRational(0).second << " "
    //         << geoTag->toRational(1).first << "/" <<
    //         geoTag->toRational(1).second << " "
    //         << geoTag->toRational(2).first << "/" <<
    //         geoTag->toRational(2).second;
...
double ExifParser::extractCaptureTime() {
    ...
    if (sscanf(time->toString().c_str(), "%d:%d:%d %d:%d:%d", &year, &month,
        ...
        return Timezone::getUTCEpoch(year, month, day, hour, minute, second,
                                     msecs, tz);
    } else {
...
bool ExifParser::extractCameraOrientation(CameraOrientation &cameraOri) {
    auto pk =
        findXmpKey({"Xmp.drone-dji.GimbalPitchDegree", "Xmp.Camera.Pitch"});
...
void ExifParser::printAllTags() {
    ...
    for (Exiv2::ExifData::const_iterator i = exifData.begin(); i != end; ++i) {
        ...
        std::cout << i->key() << " " << i->value() << " | " << tn << std::endl;
    }
}

bool ExifParser::hasExif() { return !exifData.empty(); }

bool ExifParser::hasXmp() { return !xmpData.empty(); }
