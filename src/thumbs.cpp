...
fs::path getThumbFromUserCache(const fs::path &imagePath, time_t modifiedTime,
    ...
    fs::path thumbPath =
        outdir / getThumbFilename(imagePath, modifiedTime, thumbSize);
    return generateThumb(imagePath, thumbSize, thumbPath, forceRecreate);
}

bool supportsThumbnails(EntryType type) {
    return type == EntryType::Image || type == EntryType::GeoImage ||
           type == EntryType::GeoRaster;
}

void generateThumbs(const std::vector<std::string> &input,
                    const fs::path &output, int thumbSize, bool useCrc) {
    if (!fs::is_directory(output))
        throw FSException(output.string() + " is not a valid directory");

    std::vector<fs::path> filePaths =
        std::vector<fs::path>(input.begin(), input.end());
    ...
    for (auto &fp : filePaths) {
        ...
        if (parseEntry(fp, "/", e, false, true)) {
            e.path = (fs::path("/") / fs::path(e.path))
                         .string();  // TODO: does this work on Windows?
            if (supportsThumbnails(e.type)) {
                fs::path outImagePath;
                if (useCrc) {
                    outImagePath =
                        output / getThumbFilename(e.path, e.mtime, thumbSize);
                } else {
                    outImagePath =
                        output /
                        fs::path(e.path).replace_extension(".jpg").filename();
                }
                std::cout << generateThumb(e.path, thumbSize, outImagePath,
                                           true)
                                 .string()
                          << std::endl;
            } else {
...
fs::path getThumbFilename(const fs::path &imagePath, time_t modifiedTime,
                          int thumbSize) {
...
fs::path generateThumb(const fs::path &imagePath, int thumbSize,
                       const fs::path &outImagePath, bool forceRecreate) {
    if (!fs::exists(imagePath))
        throw FSException(imagePath.string() + " does not exist");
    ...
    GDALDatasetH hSrcDataset =
        GDALOpen(imagePath.string().c_str(), GA_ReadOnly);
    if (!hSrcDataset) {
        throw GDALException("Cannot open " + imagePath.string() +
                            " for reading");
    }
    ...
    if (width > height) {
        ...
        targetHeight = static_cast<int>(
            (static_cast<float>(thumbSize) / static_cast<float>(width)) *
    ...
    CPLSetConfigOption("GDAL_PAM_ENABLED",
                       "NO");  // avoid aux files for PNG tiles
    CPLSetConfigOption(
        "GDAL_ALLOW_LARGE_LIBJPEG_MEM_ALLOC",
        "YES");  // Avoids ERROR 6: Reading this image would require libjpeg to
                 // allocate at least 107811081 bytes

    GDALTranslateOptions *psOptions = GDALTranslateOptionsNew(targs, nullptr);
    CSLDestroy(targs);
    GDALTranslate(outImagePath.string().c_str(), hSrcDataset, psOptions,
                  nullptr);
