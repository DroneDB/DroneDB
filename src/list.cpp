...
void displayEntry(const Entry& e, std::ostream& output,
...
void displayEntries(std::vector<Entry>& entries, std::ostream& output,
                    const std::string& format) {
    if (format == "text") {
        for (auto& e : entries) {
            // TODO: Tree?
...
void getBaseEntries(Database* db, std::vector<fs::path> pathList,
                    fs::path rootDirectory, bool& expandFolders,
    ...
    for (const fs::path& path : pathList) {
        ...
        expandFolders = expandFolders ||
                        pathStr.length() > 0;  // ourPath.generic() != pathStr;
        ...
        std::vector<Entry> matches =
            getMatchingEntries(db, relPath.generic(), depth + 1);

        baseEntries.insert(baseEntries.end(), matches.begin(), matches.end());
    }

    // Remove duplicates
    sort(baseEntries.begin(), baseEntries.end(),
         [](const Entry& l, const Entry& r) { return l.path < r.path; });
    baseEntries.erase(
        unique(baseEntries.begin(), baseEntries.end(),
               [](const Entry& l, const Entry& r) { return l.path == r.path; }),
        baseEntries.end());

    // Sort by type
    sort(baseEntries.begin(), baseEntries.end(),
         [](const Entry& l, const Entry& r) { return l.type < r.type; });
}

void listIndex(Database* db, const std::vector<std::string>& paths,
               std::ostream& output, const std::string& format, bool recursive,
    ...
    if (paths.empty()) {
        ...
        pathList.emplace_back(root.isParentOf(fs::current_path())
                                  ? io::Path(currentPath).generic()
                                  : directory.string());
    ...
    for (const Entry& entry : baseEntries) {
        if (entry.type != Directory)
            ...
            if (expandFolders) {
                ...
                const auto depth =
                    recursive ? maxRecursionDepth : entry.depth + 2;

                std::vector<Entry> entries =
                    getMatchingEntries(db, entry.path, depth, true);
