...
namespace ddb
{
    ...
    std::string MetaManager::entryPath(const std::string &path, const std::string &cwd) const
    {
        ...
        auto p = io::Path(path);
        if (!cwd.empty() && p.isRelative())
            p = io::Path(fs::path(cwd) / fs::path(path));
        ...
        q->bind(1, relPath);
        if (!q->fetch())
            throw InvalidArgsException("Path " + relPath + " not available in index");
    ...
    std::string MetaManager::getKey(const std::string &key, bool isList) const
    {
        ...
        if (key.empty())
            throw InvalidArgsException("Invalid empty metadata key");
        if (!utils::isLowerCase(key))
            throw InvalidArgsException("Metadata key must be lowercase");
    ...
    json MetaManager::get(const std::string &key, const std::string &path, const std::string &cwd)
    {
        ...
        return isList(key) ? result : result[0].empty() ? json::object()
                                                        : result[0];
    }
    ...
    {
        if (!ids.is_array())
            throw InvalidArgsException("ids must be an array");
