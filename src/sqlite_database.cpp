...
SqliteDatabase &SqliteDatabase::open(const std::string &file) {
    if (db != nullptr)
        throw DBException("Can't open database " + file +
                          ", one is already open (" + openFile + ")");
    LOGD << "Opening connection to " << file;
    if (sqlite3_open(file.c_str(), &db) != SQLITE_OK)
        throw DBException("Can't open database: " + file);

    //    if( sqlite3_enable_load_extension(db, 1) != SQLITE_OK ) throw
    //    DBException("Cannot enable load extension"); char *errMsg; if(
    //    sqlite3_load_extension(db, (getExeFolderPath() / "json1").c_str(),
    //    nullptr, &errMsg) == SQLITE_ERROR ) {
