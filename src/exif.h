...
struct Focal {
    ...
    DDB_DLL Focal(double length, double length35)
        : length(length), length35(length35){};
};
...
struct SensorSize {
    ...
    DDB_DLL SensorSize(double width, double height)
...
struct GeoLocation {
    ...
    DDB_DLL GeoLocation(double latitude, double longitude, double altitude)
...
struct CameraOrientation {
    ...
    double
        yaw;  // degress. 0 = magnetic north, 90 = east, -90 = west, 180 = south
    double roll;  // degrees. 20 = left roll, -20 = right roll

    DDB_DLL CameraOrientation() : pitch(0), yaw(0), roll(0){};
    DDB_DLL CameraOrientation(double pitch, double yaw, double roll)
...
class ExifParser {
   ...
   public:
    DDB_DLL ExifParser(const Exiv2::Image *image)
        : exifData(image->exifData()), xmpData(image->xmpData()){};
    DDB_DLL ExifParser(const Exiv2::ExifData &exifData,
    ...
    DDB_DLL inline double geoToDecimal(
