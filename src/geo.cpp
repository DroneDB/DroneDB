...
std::string getProjForUTM(const UTMZone &zone) {
    std::stringstream ss;
    ss << "+proj=utm +zone=" << zone.zone << " +datum=WGS84 "
       << (zone.north ? "" : "+south ") << "+units=m +no_defs";
    return ss.str();
}
...
Projected2D toUTM(double latitude, double longitude, const UTMZone &zone) {
    ...
    OGRCoordinateTransformationH hTransform =
        OCTNewCoordinateTransformation(hWgs84, hSrs);
    ...
    if (OCTTransform(hTransform, 1, &geoX, &geoY, nullptr)) {
        ...
        throw GDALException("Cannot transform coordinates to UTM " +
